...
export function PackageTable({ packages, compact = false }: PackageTableProps) {
  ...
  return <><div className="table-wrap"><table><thead><tr><th>Colis</th><th>Destinataire</th><th>Zone</th><th>Montant</th><th>Livreur</th><th>Commentaire</th><th>Statut</th><th>Mis a jour</th><th className="attempt-actions-cell">Actions</th></tr></thead><tbody>
    {displayedPackages.map((item) => <tr key={item.id}><td><strong className="tracking">{item.trackingCode}</strong></td><td>{item.recipient}</td><td>{item.city}</td><td>{item.price} DH</td><td>{item.driver ?? <span className="muted">Non affecte</span>}</td><td><div>{item.confirmationComment ?? item.importComment ?? <span className="muted">—</span>}</div>{item.returnShipmentReference && <small className="shipment-reference-value">Référence d’envoi : {item.returnShipmentReference}</small>}</td><td><span className={`status ${statusClass(item.status)}`}>{item.status}</span></td><td className="muted">{formatUpdatedAt(item.updatedAt)}</td><td className="attempt-actions-cell"><button className="text-button" onClick={() => setHistoryPackage(item)}>Historique</button></td></tr>)}
