...
const TicketOcrScanner = lazy(() => import('./components/TicketOcrScanner').then((module) => ({ default: module.TicketOcrScanner })))
...
function PackagesPage({ packages, allPackages, onImported }: { packages: DeliveryPackage[]; allPackages: DeliveryPackage[]; onImported: Refresh }) {
  ...
  function startManualPackage() {
    ...
    setForm({ trackingCode: '', storeName: '', recipient: '', phone: '', city: '', address: '', price: '', importComment: '', confirmationComment: '', packageStatus: 'MIS EN DISTRIBUTION', nextDeliveryDate: '' })
    setFormOpen(true)
    setMessage('')
  }
  ...
  function startEditPackage(item: DeliveryPackage) {
    ...
    setForm({ trackingCode: item.trackingCode, storeName: item.storeName ?? '', recipient: item.recipient, phone: item.phone ?? '', city: item.city, address: item.address, price: String(item.price), importComment: item.importComment ?? '', confirmationComment: item.confirmationComment ?? '', packageStatus: item.status, nextDeliveryDate: item.nextDeliveryDate ?? '' })
    setFormOpen(true)
    setMessage('')
  }
  ...
  function handleTicketDetected(ticket: ScannedTicket) {
    ...
    setEditingPackage(null)
    setForm({
      trackingCode: ticket.trackingCode ?? '', storeName: '', recipient: ticket.recipient ?? '', phone: ticket.phone ?? '', city: ticket.city ?? '', address: ticket.address ?? '', price: ticket.price ?? '', importComment: ticket.importComment ?? '', confirmationComment: '', packageStatus: 'MIS EN DISTRIBUTION', nextDeliveryDate: '',
    })
    setFormOpen(true)
  ...
  return <>
    ...
    {formOpen && <form className="panel package-form" onSubmit={handleSubmit}>
      ...
      <div className="package-form-grid">
        ...
        {editingPackage && form.packageStatus === 'A RECEPTIONNER' ? <label className="package-form-comment"><span>Commentaire de confirmation</span><input placeholder="Ex. Client confirmé par téléphone" value={form.confirmationComment} onChange={(event) => updateForm('confirmationComment', event.target.value)} /></label> : <label className="package-form-comment"><span>Commentaire (optionnel)</span><input placeholder="Informations complémentaires" value={form.importComment} onChange={(event) => updateForm('importComment', event.target.value)} /></label>}
      </div>
...
function DriversPage({ drivers, selectedDate, onRefresh, onViewPackages }: { drivers: Driver[]; selectedDate: string; onRefresh: Refresh; onViewPackages: (driver: Driver) => void }) {
  ...
  async function handlePdfDownload(driver: Driver) {
    setDownloadingDriverId(driver.id)
    setMessage('')
    try { await downloadDriverManifestPdf(driver.id, driver.name, selectedDate) }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Téléchargement impossible') }
    finally { setDownloadingDriverId(null) }
  }
  return <>
    ...
    <section className="assignment-grid">{pagedDrivers.map((driver) => <article className="driver-card" key={driver.id}><div className="driver-card-head"><div className="driver-avatar">{driver.initials}</div><div><h3>{driver.name}</h3><p><span className={`status ${driver.active ? 'livre' : 'retour'}`}>{driver.active ? 'Actif' : 'Inactif'}</span></p></div></div><DriverMetrics driver={driver} /><div className="card-actions"><button type="button" className="secondary-button" onClick={() => onViewPackages(driver)}>Voir les colis</button><button type="button" className="pdf-button" disabled={downloadingDriverId === driver.id} onClick={() => void handlePdfDownload(driver)}>{downloadingDriverId === driver.id ? 'Préparation…' : 'Bon PDF'}</button><button type="button" className="secondary-button" onClick={() => void startEdit(driver)}>Modifier</button><button type="button" className="danger-button" onClick={() => handleDelete(driver)}>Désactiver</button></div></article>)}{drivers.length === 0 && <div className="panel empty-state">Aucun livreur actif.</div>}</section>
    <Pagination currentPage={page} totalItems={drivers.length} pageSize={DRIVER_PAGE_SIZE} onPageChange={setPage} />
  </>
}
...
function ReturnsPage({ packages, onRefresh }: { packages: DeliveryPackage[]; onRefresh: Refresh }) {
  ...
  // A parcel may have reached the agency before the client confirmation is complete.
  // It is still physically present and can therefore be postponed or returned.
  const isReceivedAgencyReturnCandidate = (item: DeliveryPackage | null) => Boolean(item
    && item.agencyReceived
    && !item.driverId
  ...
  const directDecisionCandidates = [...pendingDecisions, ...receivedAgencyCandidates]
  const scanCandidates = [...deliveryReturnCandidates, ...directDecisionCandidates]
  ...
  async function receiveAtDepot() {
    if (!scanned || saving || !canReceiveAtDepot) return
    setSaving(true)
    try {
      const received = await registerDepotArrival(scanned.id)
      setScanned((current) => current?.id === received.id ? { ...current, ...received } : current)
      setPage(1)
      setMessage(`Colis ${received.trackingCode} réceptionné en agence. Choisissez la décision.`)
  ...
  function addShipmentPackage(item: DeliveryPackage) {
  ...
  return <>
    <div className="scanner-page-heading"><div><p className="eyebrow">FIN DE TOURNEE</p><h2>Scanner les retours</h2><p>Scannez un colis en livraison ou déjà réceptionné en agence.</p></div><div className="return-heading-indicators"><span className="status retour">{deliveryReturnCandidates.length} à recevoir</span>{directDecisionCandidates.length > 0 && <span className="pending-decision-chip">{directDecisionCandidates.length} à décider</span>}</div></div>
    {message && <p className="driver-message">{message}</p>}
    {directDecisionCandidates.length > 0 && <section className="panel pending-returns-panel"><div className="panel-heading"><h3>Décisions retour en attente</h3><span className="status retour">{directDecisionCandidates.length} à décider</span></div><div className="pending-returns-list">{directDecisionCandidates.map((item) => <button className="pending-return-item" key={item.id} onClick={() => { setScanned(item); setNextReturnDeliveryDate(''); setMessage(`Colis ${item.trackingCode} sélectionné. Choisissez une décision.`) }}><span><strong className="tracking">{item.trackingCode}</strong><small>{item.recipient} · {item.city}</small></span><span>Décider</span></button>)}</div></section>}
