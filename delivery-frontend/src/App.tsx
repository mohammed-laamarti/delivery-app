...
function PackagesPage({ selectedDate, onImported, recoveryVersion }: { selectedDate: string; onImported: Refresh; recoveryVersion: number }) {
  ...
  function handleCameraCode(trackingCode: string) {
    ...
    setMessage(`Recherche du colis ${trackingCode}.`)
  }
...
function DriverPackagesPage({ driver, selectedDate, onBack, recoveryVersion }: { driver: Driver; selectedDate: string; onBack: () => void; recoveryVersion: number }) {
  ...
  const dailyDriver = {
    ...
    // `status` is the activity for this driver and day. It remains RETOUR when a
    // depot return is kept at the agency, postponed, or made definitive, but is
    // LIVRE when the parcel was subsequently delivered.
    returns: driverPackages.filter((item) => item.status === 'RETOUR').length,
  }
